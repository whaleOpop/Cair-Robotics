...
func saveObject() :
	...
	if get("metadata/NameCell")=="Forward":
		var dict := {
			...
			"albedoColorLine": get_child(0).get("surface_material_override/0").get("albedo_color"),
			"albedoColorMesh": get_child(1).get("surface_material_override/0").get("albedo_color")
		}
		return dict
...
func loadObject(loadedDict: Dictionary) -> void:
	...
	if get("metadata/NameCell")=="Forward":
		...
		get_child(0).set("surface_material_override/0",matBlack)
		get_child(1).set("surface_material_override/0",matWhite)
	elif get("metadata/NameCell")=="Rotate":
		position =parseVec3(loadedDict.position)
		rotation_degrees = parseVec3(loadedDict.rotation_degrees)
		matBlack.albedo_color=parseVec4(loadedDict.albedoColorLine)
